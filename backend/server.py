...
async def debug_templates(request: Request):
    """Temporary debug endpoint to diagnose template issues - NO AUTH"""
    try:
        # Count all templates
        ...
        # Get all unique organization_ids in templates
        all_org_ids = await db.observation_templates.distinct("organization_id")
        
        # Get count of templates without organization_id
        templates_no_org = await db.observation_templates.count_documents({"organization_id": {"$exists": False}})
        templates_null_org = await db.observation_templates.count_documents({"organization_id": None})
        
        # Get sample users with their org_ids
        sample_users = await db.users.find({}, {"_id": 0, "user_id": 1, "email": 1, "organization_id": 1}).limit(5).to_list(5)
        
        return {
            "total_observation_templates": total_obs_templates,
            "total_reflection_templates": total_ref_templates,
            "templates_without_org_id": templates_no_org,
            "templates_with_null_org_id": templates_null_org,
            "unique_organization_ids_in_templates": all_org_ids,
