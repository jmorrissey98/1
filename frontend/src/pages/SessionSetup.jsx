...
export default function SessionSetup() {
  ...
  const loadObservationTemplates = async (applyDefault = false) => {
    ...
    try {
      ...
      if (apiTemplates && apiTemplates.length > 0) {
        ...
        // No templates from API - fall back to localStorage (matches TemplateManager behavior)
        console.warn('[SessionSetup] No templates from API, using localStorage fallback');
        const localTemplates = storage.getTemplates();
        if (localTemplates && localTemplates.length > 0) {
          console.log('[SessionSetup] Using localStorage templates:', localTemplates.length);
          ...
          // No templates anywhere - show empty state
          console.warn('[SessionSetup] No templates available anywhere');
          setTemplates([]);
        }
      }
    } catch (err) {
      console.error('Failed to load observation templates:', err);
      // Fallback to localStorage on error (matches TemplateManager behavior)
      console.log('[SessionSetup] Error - using localStorage templates as fallback');
      const localTemplates = storage.getTemplates();
      if (localTemplates && localTemplates.length > 0) {
        setTemplates(localTemplates);
        setSelectedTemplate(localTemplates[0].id || 'default');
  ...
  const loadCoaches = async () => {
    try {
      ...
      if (result.ok && result.data) {
        ...
        // Phase 4: Fetch observation limits for all coaches
        const coachIds = result.data.map(c => c.id);
        if (coachIds.length > 0) {
          setLoadingLimits(true);
          try {
            const limitsResult = await fetchBulkCoachObservationStatus(coachIds);
            if (limitsResult.ok && limitsResult.data) {
              setCoachObservationStatus(limitsResult.data);
            }
          } catch (limitErr) {
            console.error('[SessionSetup] Failed to load observation limits:', limitErr);
          } finally {
  ...
  const handleStartObservation = async () => {
    ...
    // Phase 4: Check observation limit before starting
    const status = coachObservationStatus[selectedCoachId];
    if (status) {
      const canObserve = canObserveCoach(status);
      if (!canObserve.allowed) {
        toast.error(canObserve.reason || 'Observation limit reached for this coach');
        return;
      }
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-4xl mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
        ...
        {/* Session Parts - From Template */}
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="font-['Manrope']">Session Parts</CardTitle>
              <CardDescription>Parts from selected template. You can edit names or add custom parts.</CardDescription>
            </div>
            <Button
              size="sm"
              variant="outline"
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              {(session.sessionParts || []).length === 0 ? (
                <p className="text-sm text-slate-400 italic py-4">No parts yet. Add a part to get started.</p>
              ) : (
                <div className="space-y-2">
